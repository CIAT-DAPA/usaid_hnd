...
varList <- c( "rain")
bDir <- "D:/col-usaid/02-monthly-interpolations/outputs_yearlyv_nov"
...
for (yr in years){
  ...
  for (i in 1:length(varList)){
    ...
    for (mth in 1:12) {
     ...
     ens<-list()
        for ( nf in 1:nfolds){
          ens[i]<-raster(unzip(paste(paste(iDir, "/", yr, "/fold-",nf, "/tile-1/", varList[i], "_", mth, ".zip", sep=""))))
        }
        mthStack <- stack(ens)
...
bDir <- "D:/col-usaid/02-monthly-interpolations/outputs_yearlyv_nov/average"
#years <- 1980:2010
oDir <- bDir
