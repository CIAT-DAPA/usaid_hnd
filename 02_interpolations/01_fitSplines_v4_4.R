...
setwd("D:/cenavarro/hnd_pnud/02_interpolations")
anuDir <- "D:/cenavarro/hnd_pnud/02_interpolations/anu/Anuspl44/bin"
stDir <- "W:/01_weather_stations/pnud_hnd/merge/combined"
rDir <- "D:/cenavarro/hnd_pnud/region"
oDir <- "D:/cenavarro/hnd_pnud/interpolations/tmax"
...
require(foreign)
#rm(list=ls());gc(T);gc()
source("writeDatFile.R"); source("createFitFile_v4_4.R"); source("createValFile.R"); source("createPrjFile.R"); source("accuracy.R")
...
splineFitting <- function(anuDir="C:/Users/jardila/Desktop/monthly-interpolations/anu/Anuspl43/bin", stDir, rDir, oDir, nfolds=25, train.per=0.85, vn="rain", ntiles=1, unix=F, suffix="some") {
  ...
  st <- read.csv(paste(stDir,"/", vn, "_", period,".csv",sep="")); st$.id <- NULL
  ...
  for (fold in 1:nfolds) {
    ...
    for (tile in 1:ntiles) {
      ...
      if (!file.exists(paste(vn, "-status.anu", sep=""))) {
        ...
        nk <- nrow(st.sel.10y[which(st.sel.10y$TRAIN_TEST == "TRAIN"),])
        
        #Creating run file
        cat("Running \n")
        createRunFile(st.sel.10y, filename=paste(vn, "_train.dat", sep=""), variable=vn, 
                      varUnits=u, ivars=3, icovars=0, sivars=0, sicovars=0, sp.order=3, nsurf=12, knots=nk)
        #Running
        if (unix) {
          fw <- file(paste(vn, "_fit.sh", sep=""), open="w")
          cat("wine ", anuDir, "/spline.exe < ", vn, "fit.cmd > ", vn, "fit.log\n", sep="", file=fw)
          close(fw)
          system(paste("sh ", vn, "_fit.sh", sep=""))
        } else {
          fw <- file(paste(vn, "_fit.bat", sep=""), open="w")
          cat(anuDir, "/spline < ", vn, "fit.cmd > ", vn, "fit.log\n", sep="", file=fw)
          close(fw)
