...
# Set params
bDir <- "D:/cenavarro/hnd_usaid/04_interpolation/outputs_yearly_v2"
...
for (vn in varList){
  ...
  write.csv(metrics, paste(oDir, "/", vn, "_metrics.csv", sep=""), row.names=F)
}

setwd(oDir)

id_rain <- read.csv(paste("rain_metrics.csv", sep=""))
id_tmax <- read.csv(paste("tmax_metrics.csv", sep=""))
id_tmin <- read.csv(paste("tmin_metrics.csv", sep=""))
oPlot <- paste0(oDir, '/skill.tif') 
...
title("Maximum Temperature", line = 1)
grid()
lines(c(0,13),c(1 *.5, 1 *.5),col='red',lty=2)
lines(c(0,13),c(1 *.75,1 *.75),col='red')
...
# text(x=11.9, y=0.98,"D", pos=4, cex=1.1.4)
title("Minimum Temperature", line = 1)
grid()
lines(c(0,13),c(1 *.5, 1 *.5),col='red',lty=2)
lines(c(0,13),c(1 *.75,1 *.75),col='red')
...
par(mar=c(4,4,2,1))
boxplot(id_tmax$RMSE.TEST/10 ~ id_tmax$MONTH,xlab='Month',ylab='RMSE (ºC)',pch=20,cex=1.1,ylim=c(0,0.5), naMonth=(mth_lb), cex.axis=0.9, col="gray90")
...
# tiff('fig-3f-rmse_tmean_monthly.tif',width=600, height=600,pointsize=8,compression='lzw',res=150)
par(mar=c(4,4,2,1))
boxplot(id_tmin$RMSE.TEST/10 ~ id_tmin$MONTH,xlab='Month',ylab='RMSE (ºC)',pch=20,cex=1.1,ylim=c(0,0.5), naMonth=(mth_lb), cex.axis=0.9, col="gray90")
# text(x=11.9, y=0.98,"F", pos=4, cex=1.1.4)
grid()
