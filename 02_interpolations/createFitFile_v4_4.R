...
varUnits="metres", ivars=3, icovars=0, sivars=0, sicovars=0, sp.order=2, nsurf=12, knots=100) {
  #Units matrix
  ...
  cat(knots, "\n",sep="",file=fw) #Knots
  cat("\n",sep="",file=fw) #Output knot index file name
  cat("\n",sep="",file=fw) #Input bad data flag file
  cat("\n",sep="",file=fw) #Output bag data flag file
  cat(variable,".res\n", sep="", file=fw) #Residual data file
  cat(variable,".lcv\n", sep="", file=fw) #Large cross validation file
  ...
  cat("\n", sep="", file=fw) #Cross validation file
  cat("\n",file=fw)
