...
dtr_tean <- function(bDir, sY, fY, oDir){
  ...
  for (y in sY:fY){
    ...
    yrdata_info <- read.csv(paste(bDir,"/tmax_", y, ".csv"  ,sep=""))
    cols <- colnames(yrdata_info) %in% toupper(mthLs)
        
    # Read all files
    yrdata_tx <- subset(read.csv(paste(bDir,"/tmax_", y, ".csv"  ,sep="")),,cols)
    yrdata_tn <- subset(read.csv(paste(bDir,"/tmin_", y, ".csv"  ,sep="")),,cols)
    
    yrdata_dtr <- yrdata_tx - yrdata_tn
    yrdata_dtr[yrdata_dtr < 1] <- 1
    
    yrdata_tavg <- (yrdata_tx + yrdata_tn)/2
    
    write.csv(cbind(yrdata_info[,1:8], yrdata_dtr, NYEARS=yrdata_info[,21]), paste0(oDir, "/dter_", y, ".csv"), row.names=F)
    write.csv(cbind(yrdata_info[,1:8], yrdata_tavg, NYEARS=yrdata_info[,21]), paste0(oDir, "/tean_", y, ".csv"), row.names=F)
...
oDir <- "W:/04_interpolation/stations-averages/yearly_pseudost_v2"
...
# 20-yr clim
bDir <- "W:/04_interpolation/stations-averages/yearly_pseudost_v2"
varList <- c("tmax", "tmin")
sY <- 1991
fY <- 2010
oDir <- "D:/cenavarro/hnd_usaid/04_interpolation/stations-averages/climatology_v2"
for (var in varList){
  climatology(bDir, var, sY, fY, oDir)
}


# Dtr and tean yeraly calculation
bDir <- "W:/04_interpolation/stations-averages/yearly_pseudost_v2"
sY <- 1990
fY <- 2014
oDir <- "W:/04_interpolation/stations-averages/yearly_pseudost_v2"
dtr_tean(bDir, sY, fY, oDir)
