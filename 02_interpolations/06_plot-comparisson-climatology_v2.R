...
# Set params
bDir <- "D:/OneDrive - CGIAR/CIAT/Projects/hnd-pnud-ciat-escenarios/03_Interpolacion/average"
oDir <- "D:/OneDrive - CGIAR/CIAT/Projects/hnd-pnud-ciat-escenarios/03_Interpolacion/performance"
varList <- c("prec") #, "tmean", "dtr", "tmax", "tmin")
id <- c("DJF", "MAM", "JJA", "SON")
id_mod <-c("DEF", "MAM", "JJA", "SON")
# mask <-readOGR("D:/cenavarro/hnd_pnud/region/Limites_Honduras_v3.shp",layer="Limites_Honduras_v3")
mask <- readOGR("D:/OneDrive - CGIAR/CIAT/Projects/hnd-pnud-ciat-escenarios/03_Interpolacion/region/Limites_Departamentos.shp")
...
for (var in varList){
  ...
  if (var == "prec"){
    ...
    zvalues <- seq(0, 1200, 40) # Define limits
    myTheme <- BuRdTheme() # Define squeme of colors
    myTheme$regions$col=colorRampPalette(c("khaki1", "skyblue1", "blue", "darkblue", "darkmagenta"))(length(zvalues)-1) # Set new colors
    myTheme$strip.border$col = "white" # Eliminate frame from maps
    myTheme$axis.line$col = 'white' # Eliminate frame from maps
  ...
  print(levelplot(plot, at = zvalues, scales = list(draw=FALSE), layout=c(2, 2), xlab="", ylab="", par.settings = myTheme, 
  ...
  if (var == "prec"){
    ...
    grid.text(expression("°C"), 0.2, 0, hjust=8, vjust=-5, gp=gpar(fontsize=12))  
  }
