...
minim = 0 ;maxim = 50; ric = 7; criterio1=15
...
if(variable=="tmax"){
  ...
  for(i in 1:(ncol(object)-3))
  {
    ...
    if(na<0.95){
      ...
      tiff(paste0(outDir,"quality_control/tmax/boxplot_graph/",nomb[i],"_",variable,"_box_graph_qc.tiff"),compression = 'lzw',height = 7,width = 10,units="in", res=150)
      par(mfrow = c(1,2),oma = c(0, 0, 2, 0),las=1)
      ...
      tiff(paste0(outDir,"quality_control/tmax/line_graph/",nomb[i],"_",variable,"_line_graph_qc.tiff"),compression = 'lzw',height = 7,width = 16,units="in", res=150)
      ...
      ylim1 = c(min(data_station[,i+3],na.rm=T) ,max(data_station[,i+3],na.rm=T)+1)
      ylim2 = c(min(object[,i+3],na.rm=T) ,max(object[,i+3],na.rm=T)+1)
      
      plot(data_station[,i+3],type="l",col="grey",xaxt="n",yaxt="n",xlab="",ylab="Temperatura máxima (°C)",main = "Antes del control de calidad",ylim=ylim1,cex.lab=0.8)
      
      mtext(paste0("Estación ",name_st[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2) 
      points(out_range,data_station[out_range,i+3],col="black",bg="green",cex=0.7,pch=21)
      points(out_cons,data_station[out_cons,i+3],col="black",bg="red",cex=0.7,pch=21)
      points(c(out_atip.na.y,out_atip.na),data_station[c(out_atip.na.y,out_atip.na),i+3],col="black",bg="purple",cex=0.7,pch=21)
      points(out_salt.n,data_station[out_salt.n,i+3],col="black",bg="turquoise1",cex=0.7,pch=24)
      ...
      add_legend("bottomright", legend=c("Datos consecutivos","Datos atípicos","Datos por fuera del rango","Saltos consecutivos"), pch=c(21,21,21,24), 
                 pt.bg= c("red","purple","green","turquoise1","tan3"),bty = "n",cex = 0.9,
                 horiz=TRUE)
