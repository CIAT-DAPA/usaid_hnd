...
iDir <- paste0(net_drive, "/Water_Planning_System/Inputs/WPS/Balance_Hidrico/shared") # This line goes with line # 16
varLs <- c("prec", "eto", "tmax", "tmin", "tmean")
varRs <- c("Precipitación", "Evapotranspiración Potencial", "Temperatura Máxima", "Temperatura Mínima", "Temperatura Promedio")

#iDir <- paste0(net_drive, "/10_outputs/WPS/Balance_Hidrico/thornthwaite_and_mather/", scenario)
...
mask_shp <- paste0("S:/06_analysis/Scenarios/mask/ZOI.shp")
...
for (var in varLs){
  ...
  if (var %in% c("tmax", "tmin", "tmean")){
    ...
    mytheme = rasterTheme(region = RColorBrewer::brewer.pal(9, "YlOrRd"))
    units = "ºC"
  }
  else{
    # Define symbology of blues. Number of intervals has to be equal to length(zvalues)-1
    mytheme = rasterTheme(region = RColorBrewer::brewer.pal(9, "Blues"))
    units = "mm"
  }
  ...
  # Raster stack
  #rs_stk <- stack(paste0(iDir, "/", var, "/projected/",  var, "_month_", 1:12, ".tif"))
