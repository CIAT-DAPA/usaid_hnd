...
r13_dir = r"\\dapadfs\workspace_cluster_7\Ecosystem_Services\Water_Planning_System\COSUDE_R13"
scenario = "rcp8.5_2050"
thornthwaite_and_mather_dir = "thornthwaite_and_mather"
shared_dir = os.path.join(in_dir, "shared") # For baseline
cc_dir  = os.path.join(in_dir, "climate_change", "downscaled_ensemble", scenario)
cc_sce = ["rcp2.6_2030", "rcp2.6_2050", "rcp8.5_2030", "rcp8.5_2050"]
tam_in_dir = os.path.join(in_dir, thornthwaite_and_mather_dir)
tables_dir = os.path.join(tam_in_dir, 'tables')
out_dir = os.path.join(r13_dir, "10_outputs\WPS\Balance_Hidrico")
tam_out_dir = os.path.join(out_dir, thornthwaite_and_mather_dir, scenario)
...
#k = 0.76
k_rec = Float(Raster(os.path.join(r13_dir, "07_inputs\WPS\Balance_Hidrico", thornthwaite_and_mather_dir, "k_recession." + wildcard)))
k_may_ago = Float(Raster(os.path.join(r13_dir, "07_inputs\WPS\Balance_Hidrico", thornthwaite_and_mather_dir, "k_may_ago." + wildcard)))
k_nov_mar = Float(Raster(os.path.join(r13_dir, "07_inputs\WPS\Balance_Hidrico", thornthwaite_and_mather_dir, "k_nov_mar." + wildcard)))
#b = Float(Raster(os.path.join(tam_in_dir, 'b_runoff.' + wildcard)))
b_esc = 0.208
b_may_ago = Float(Raster(os.path.join(r13_dir, "07_inputs\WPS\Balance_Hidrico", thornthwaite_and_mather_dir, "b_may_ago." + wildcard)))
b_nov_mar = Float(Raster(os.path.join(r13_dir, "07_inputs\WPS\Balance_Hidrico", thornthwaite_and_mather_dir, "b_nov_mar." + wildcard)))
...
zoi = os.path.join(r13_dir, "03_geodata\ZOI_R13.gdb", "ZOI_Incl_Goas")  # Zone of Interest: Microwatersheds dissolved for R13 including upper Goascoran
...
def choose_input_folder(sce, yr):
    if sce == "baseline":
        precip_dir = os.path.join(shared_dir, weather_vars['precipitation'], 'projected', str(yr))
        eto_dir = os.path.join(shared_dir, weather_vars['potential_evap'], 'projected', str(yr))
        return(precip_dir, eto_dir)
    elif sce in cc_sce:
        precip_dir = os.path.join(cc_dir, weather_vars['precipitation'], str(yr))
        eto_dir = os.path.join(cc_dir, weather_vars['potential_evap'], str(yr))
        return(precip_dir, eto_dir)
    else:
        print "ERROR: Defined scenario is not a possible option!!"
        exit()
...
for year in years_execution:
    ...
    precip_dir, eto_dir = choose_input_folder(scenario, year)
    ...
    for month in months_execution:
        ...
        # Define which k and b to use depending on season
        if month in [5, 6, 7, 8]:
            k = k_may_ago
            b = b_may_ago
