### Merge CSV files resulting of the script "07_zonal_statistics_monthly.R" (Not used anymore but possible if desired: calculate water yield (wyield) and summarize variables on annual basis)
...
# List of CVS files with a specific pattern. Looks for files with names starting with "mth_avg" and ending with "filled.csv" 
CSVs = list.files(pattern = "mth_avg.+filled.csv")
...
for (csv in CSVs){
  ...
  var = unlist(strsplit(tail(strsplit(csv, "_")[[1]], 2), "[.]"))[1]
