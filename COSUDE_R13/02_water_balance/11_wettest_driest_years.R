...
prec = read.csv(paste0(net_drive, "/06_analysis/Scenarios/", scenario, "/mth_yearly_timeline_prec_filled.csv"))
...
wyield_newcols =  c("HydroID", "Ano", paste0("wyield_", months))
...
for (i in 1:length(prec[,1])){
  ...
  # Get the row (HydroID of the microwatershed) from the wyield data
  match_row2 = which(wyield[1] == id)
  
  # Get the values for that year
  wyield_mincols = paste0("wyield_", min_year, "_", months)
  wyield_maxcols = paste0("wyield_", max_year, "_", months)
  wyield_minvs = wyield[match_row2, wyield_mincols]
  wyield_maxvs = wyield[match_row2, wyield_maxcols]
  wyield_mincolstoadd = cbind(id, min_year, wyield_minvs)
  wyield_maxcolstoadd = cbind(id, max_year, wyield_maxvs)
  names(wyield_mincolstoadd) = wyield_newcols
  names(wyield_maxcolstoadd) = wyield_newcols
  ...
  if (i == 1){
    ...
    wyield_mindt = wyield_mincolstoadd
    wyield_maxdt = wyield_maxcolstoadd
  }
  else{
    ...
    wyield_mindt = rbind(wyield_mindt, wyield_mincolstoadd)
    wyield_maxdt = rbind(wyield_maxdt, wyield_maxcolstoadd)
  }
}
...
write.csv(wyield_mindt, paste0(oDir, "/driest_year_wyield.csv"), row.names = FALSE)
write.csv(wyield_maxdt, paste0(oDir, "/wettest_year_wyield.csv"), row.names = FALSE)