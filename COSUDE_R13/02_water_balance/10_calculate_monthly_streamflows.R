...
wyield = read.csv(paste0(net_drive, "/06_analysis/Scenarios/", scenario, "/mth_avg_timeline_wyield_inclu_goas_filled.csv"))
...
for (i in 1:length(str_net$id)){
  ...
  if (id %in% wyield[,1]){
    ...
    monthly_flow_m3s = (wyield[match_row, (num_columns-11):(num_columns)]/1000)*area_m2/(30.42*86400)
    ...
    if (upstr_cats[1] == ""){
      ...
      # Average by columns
      avg_cats = apply(row_cats[,(num_columns-11):(num_columns)], 2, mean)
...
write.csv(as.matrix(flow_data_R13), paste0(oDir, "/mth_avg_timeline_sflow.csv"), row.names = FALSE)
