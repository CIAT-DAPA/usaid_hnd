...
if(!file.exists(paste0(oDir, "/ann_chgs_data_regions.csv"))){
  ...
  for (i in 1:length(as.vector(regions$REGION)) ){
    ...
    for (var in varLs){
      ...
      anom_stat <- cellStats(anom_crop, stat='median', na.rm=TRUE)
  ...
  write.csv(mt, paste0(oDir, "/annual_data_regions.csv"), row.names = F)
  
}


annData <- read.csv(paste0(oDir, "/annual_data_regions.csv"), header = T)
...
for (i in 1:length(as.vector(regions$REGION)) ){
  ...
  p <- ggplot(annData_rg_var, aes(x=Period, y=Median, group=RCP, color=RCP)) +
    scale_color_manual(name="",
                       ...
                       values=c("steelblue1","steelblue2","steelblue3", "steelblue4")) +
  ...
  p <- ggplot(annData_rg_var, aes(x=Period, y=Median, group=RCP, color=RCP)) +
    scale_color_manual(name="",
                       labels = c("RCP 2.6", "RCP 4.5", "RCP 6.0", "RCP 8.5"),
                       values=c("firebrick1","firebrick2","firebrick3", "firebrick4")) +
    geom_line(aes(color=RCP), size=0.9) +
    scale_x_discrete(labels=c("2020_2049" = "2030s", "2040_2069" = "2050s",
                              "2070_2099" = "2080s"),
                     expand = c(0.05, 0.05)) +
