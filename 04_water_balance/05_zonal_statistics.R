...
# Arguments
# iDir <- "W:/Inputs/WPS/Balance_Hidrico/shared"
# varLs <- c("tmin") #, "tmean", "tmin"
...
for (var in varLs){
  ...
  rs_stk_crop <- crop(rs_stk_diss, extent(poly))
...
for (var in varLs){
  ...
  # Raster stack
  rs_stk <- stack(paste0(iDir, "/", var, "/projected/", yr_mth[,2], "/", var, "_", yr_mth[,2], "_", 1:12, ".tif"))
